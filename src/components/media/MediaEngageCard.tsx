...
const MediaEngageCard = ({ title, description, image, link, badge }: MediaEngageCardProps) => {
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-[700px] sm:h-[800px] lg:h-[950px] flex flex-col">
            ...
            <div className="p-4 sm:p-6 lg:p-8 bg-[#C4B4D1] flex flex-col justify-between flex-grow">
                <div className="flex flex-col gap-3 sm:gap-4 lg:gap-6">
                    {/* Badge */}
                    <div className="min-h-[1.5rem] sm:min-h-[2rem] flex items-start">
                        {badge && (
                            <span className="inline-block bg-white bg-opacity-90 text-[#7A7A7A] text-xs sm:text-sm font-medium px-2 sm:px-3 py-1 rounded-lg">
                                {badge}
                            </span>
                        )}
                    </div>
...
export default MediaEngageCard;